    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.card}>
        <View style={styles.headerRow}>
          ...
          <Text style={styles.vehicleTitle}>{vehicle.brand} {vehicle.name}</Text>
        </View>
      ...
      {/* Szczegóły pojazdu */}
        <Text style={styles.detailLabel}>Rejestracja: <Text style={styles.detailValue}>{vehicle.plate}</Text></Text>
        <Text style={styles.detailLabel}>Rocznik: <Text style={styles.detailValue}>{vehicle.year}</Text></Text>
        <Text style={styles.detailLabel}>Pojemność baku: <Text style={styles.detailValue}>{vehicle.tankCapacity} L</Text></Text>
        <Text style={styles.detailLabel}>Paliwo: <Text style={styles.detailValue}>{vehicle.fuelType}</Text></Text>
        <Text style={styles.detailLabel}>Przebieg: <Text style={styles.detailValue}>{currentKm} km</Text></Text>

        {/* Zdjęcia dokumentów */}
        <View style={{ marginTop: 20 }}>
          <Text style={{ fontWeight: 'bold', fontSize: 16, marginBottom: 10, color: '#23263b' }}>Dokumenty pojazdu</Text>
          {vehicle.ocPhoto && (
            <View style={{ marginBottom: 14 }}>
              <Text style={{ fontWeight: '600', color: '#3478f6', marginBottom: 6 }}>Polisa OC</Text>
              <TouchableOpacity onPress={() => openImageModal(vehicle.ocPhoto!)}>
                <Image source={{ uri: vehicle.ocPhoto }} style={{ width: 140, height: 100, borderRadius: 8 }} />
              </TouchableOpacity>
            </View>
          )}
          {vehicle.regPhoto && (
            <View style={{ marginBottom: 14 }}>
              <Text style={{ fontWeight: '600', color: '#3478f6', marginBottom: 6 }}>Dowód rejestracyjny</Text>
              <TouchableOpacity onPress={() => openImageModal(vehicle.regPhoto!)}>
                <Image source={{ uri: vehicle.regPhoto }} style={{ width: 140, height: 100, borderRadius: 8 }} />
              </TouchableOpacity>
            </View>
          )}
        </View>
        ...
        {sortedEntries.length === 0 ? (
          ...
          sortedEntries.map((item) => (
            <View style={styles.historyItem} key={item.id}>
              <MaterialCommunityIcons name="fuel" size={22} color="#30d158" style={{ marginRight: 8 }} />
              <View style={{ flex: 1 }}>
      ...
      {/* Modal pełnoekranowy dla zdjęcia */}
